...
link, deprecate) → validate it against the overlaid in-memory bundle → apply it
atomically. Prospective validation runs the same validator on `bundle ⊕ change`
— with whatever rule set is registered in the catalog — so refusals carry the
same Report the CLI produces, by construction. The implementation hides the
bundle overlay, lossless concept round-trip, index/log maintenance (through the
ADR-0004 model), and multi-file staging with rollback.
...
- No second checker: prospective validation cannot drift from `okf validate`.
