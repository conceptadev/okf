...
| §5 | Every timestamp is an ISO 8601 datetime with a UTC offset. Date-only values from earlier revisions are read as midnight UTC. | `okf/timestamp-without-offset` advisory; still conformant. `okf format --migrate-timestamps` rewrites date-only values. | `test/timestamps_test.dart`, `test/cli_test.dart` |
